...
def dataloader_spring(batch_size=1, sim_folder="", data_folder="data", len_enc=49, len_dec=50, noise_var=0.):
...
def dataloader_kuramoto(batch_size=1, sim_folder="", data_folder="data", len_enc=50, len_dec=50, noise_var=0., ww=True, distributed=True):
    ### the edges numpy arrays below are [ num_sims, N, N ]
...
def MLP_layers(cfg, nl_type, batch_norm=0, dropout=0):
    layers = []
    ...
    for i in range(1, len(cfg)):
        if i != len(cfg) - 1:
            layers += [('FC' + str(i) + '0', nn.Linear(cfg[i - 1], cfg[i], bias=bias))]
            if batch_norm:
            ...
            layers += [('FC' + str(i) + '0', nn.Linear(cfg[i - 1], cfg[i], bias=False))]

    return nn.Sequential(OrderedDict(layers))


def Res_layers(cfg, nl_type, batch_norm=0, dropout=0):
    meta_layers = []
    nl_dict = {'RL': nn.ReLU(), 'TH': nn.Tanh(), 'LR': nn.LeakyReLU(0.2), 'GL' : nn.GELU(), 'SL' : nn.SELU(), 'MS' : mish()}
    nl = nl_dict[nl_type]
    bias=True if len(cfg) > 2 else False
...
class MLP_Block(nn.Module):
    def __init__(self, cfg, nl_type, batch_norm=0, dropout=0):
        super(MLP_Block, self).__init__()
        self.FC = MLP_layers(cfg, nl_type, batch_norm, dropout)
...
def cfg_Block(block_type, cfg, nl_type, batch_norm=0, dropout=0):
    if block_type == 'mlp':
        block = MLP_Block(cfg, nl_type, batch_norm, dropout)
    elif block_type =='res':
